...
GitHub Pages 会把网页缓存 **10 分钟**（`Cache-Control: max-age=600`）。
如果改了网站但访客看到的还是旧版，**根本原因是缓存**，解决办法是改版本号：

**需要改 3 个地方**（把 4 换成新数字，每次递增）：

1. `js/data.js` → `version: '4'` 改成 `version: '5'`
2. `index.html` → 所有 `?v=4` 改成 `?v=5`
3. `watch.html` → 所有 `?v=4` 改成 `?v=5`

原理：**首页的作品链接会自动带上版本号**（见 `js/home.js`），
版本号一变，访客的浏览器就认为这是个新网址，会重新下载所有文件，绕开缓存。

> 💡 跟 AI 说"我更新了网站，帮我升一下版本号"最省事。

**如果访客还卡在旧版**，让他用**无痕/隐私模式**打开，或者清除浏览器缓存。

---

## 🎮 播放器怎么操作

播放器会**自动识别设备** —— 手机用手机的习惯，电脑用电脑的习惯。

### 📱 手机上

| 操作 | 效果 |
|---|---|
| **单击**画面 | 显示控制条；再点一次收起。（播放中显示后约 3 秒会自动收起） |
| **双击**画面 | 播放 / 暂停，中间会弹出圆形图标提示 |
| **点全屏按钮** | 进入全屏，**安卓会自动转横屏**（iPhone 受系统限制不会，需要手动横过来） |
| 拖进度条 | 手指按住拖动，上方会显示对应时间 |
